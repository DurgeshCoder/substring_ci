...
<script>
   ...
   function courseChange() {
    ...
    const topicTable = document.getElementById('batch_table');
    
    // Clear existing rows in the table
    topicTable.innerHTML = '';

    // Fetch batches for the selected course
    get_batches_of_course(selectedCourseId).then(data => {
        if (data.length === 0) {
            // If there are no topics, you can display a message
            const noBatchRow = document.createElement("tr");
            noBatchRow.innerHTML = '<td colspan="4">No batches available for the selected course.</td>';
            topicTable.appendChild(noBatchRow);
        } else {
            ...
            for (let t of data) {
                ...
                const batch_name = document.createElement("td");
                batch_name.className='px-6 py-4 w-80 font-medium text-blue-800 text-lg dark:text-gray-100';
                batch_name.textContent = t.name;

                const description = document.createElement("td");
                description.className='px-6 py-4 dark:text-gray-100';
                description.textContent = t.short_description;
                ...
                a_view.href = "batch_view/"+t.batch_id;
                ...
                a_edit.href = "edit_batch/"+t.batch_id;
                ...
                button_delete.value=t.batch_id
...
<!-- message end -->
<h1 class="text-center bg-white text-4xl font-bold py-4 text-blue-800 dark:text-gray-100 dark:bg-gray-800">Batches</h1>
    ...
    <table class="w-full text-base text-left text-gray-500 dark:text-gray-400 " id="content">
        ...
        <tbody id="batch_table">
            ...
            <tr class="bg-white border-b  dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td id="batch_name" class="px-6 py-4 w-80 font-medium text-blue-700 text-lg dark:text-gray-100">
                <?=$batch['name']?>
                </td>
                ...
                <td class="px-6 py-4 text-center flex justify-center">
                    ...
                    <a href="<?php echo base_url('admin/batch_view/'.$batch['batch_id']) ?>" class="font-medium text-blue-600 dark:text-blue-500 hover:underline mx-2"><button type="button" class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2 mr-2 mb-2 dark:focus:ring-yellow-900"><i class="fa-solid fa-eye fa-lg"></i></button></a>

                    <a href="<?php echo base_url('admin/edit_batch/'.$batch['batch_id']) ?>" class="font-medium text-blue-600 dark:text-blue-500 hover:underline mx-2"><button type="button" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"><i class="fa-solid fa-pen-to-square fa-lg"></i></button></a>

                    <a href="#" class="font-medium text-blue-600 dark:text-blue-500 hover:underline mx-2"><button type="button" value="<?= $batch['batch_id']?>" class="confirm_del_btn focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2 mr-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900"><i class="fa-solid fa-trash fa-lg"></i></button></a> 
